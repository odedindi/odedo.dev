define('./workbox-202dc43a.js', ['exports'], function (exports) {
    ...
    */
	...
	function registerRoute(capture, handler, method) {
		...
		if (typeof capture === 'string') {
			...
			{
				if (
					!(capture.startsWith('/') || capture.startsWith('http'))
				) {
    ...
    */
	const _cacheNameDetails = {
		...
		runtime: 'runtime',
		suffix:
			typeof registration !== 'undefined' ? registration.scope : '',
	};
    ...
    */
	...
	async function cacheMatchIgnoreParams(
		...
		for (const cacheKey of cacheKeys) {
			const strippedCacheKeyURL = stripParams(
				cacheKey.url,
				ignoreParams,
			);
    ...
    */
	...
	class StrategyHandler {
		...
		async cacheMatch(key) {
			...
			const effectiveRequest = await this.getCacheKey(
				request,
				'read',
			);
		...
		async cachePut(key, response) {
			...
			const hasCacheUpdateCallback =
				this.hasCallback('cacheDidUpdate');
